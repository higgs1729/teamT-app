...
- WebAPI Gallery : `templates/`内の各WebAPI紹介ページ(自己完結型HTML)を選択して表示するギャラリー画面
  ...
  - おすすめ一覧: 検索欄の直下に配置し、初めて開いた人が試しやすいAPIを `fronted-v2/js/app.js` の固定IDリストで抜粋表示する。表示名・アイコン・選択動作は `CATALOG` の該当項目を参照し、検索中は一致するおすすめだけに絞り込む。
  - サイドバー項目の行レイアウト: おすすめ一覧・カテゴリ別一覧(nav-list)の各項目は「アイコン／項目名／右向き矢印(chevron-right、常に右向き固定)」の順で表示する。折りたたみ見出し(おすすめ一覧の開閉ヘッダー、カテゴリ・小分類の開閉ヘッダー)の開閉キャレットは別要素のため対象外（開閉状態に応じて回転する）。
