...
- WebAPI Gallery : `templates/`内の各WebAPI紹介ページ(自己完結型HTML)を選択して表示するギャラリー画面
  ...
  - おすすめ一覧: 検索欄の直下に配置。見出しは開閉ボタンではなく大見出し（`<h2 class="recommend-heading">`）のみで、常に展開表示する。表示対象は `../おすすめ一覧.txt`（各メンバーが推薦するテンプレートのファイル名を1行ずつ記載）を起動時に読み込み、`.html`で終わる行だけを`CATALOG`の`file`（ディレクトリを除いたbasename・大小無視）と突き合わせて抽出する。名前欄やコメント行、一致しないファイル名は無視される。表示名・アイコン・選択動作は`CATALOG`の該当項目を参照し、検索中は一致するおすすめだけに絞り込む。
  - サイドバー項目の行レイアウト: 「アイコン／項目名／右向き矢印(chevron-right、常に右向き固定・回転しない)」の＞は、最内アイテム（おすすめ一覧の各項目・nav-listの個々のAPI項目）ではなく、それ以外の折りたたみ行（カテゴリ見出し・小分類見出し）の行末に付ける。カテゴリ見出しは「カテゴリ名 ＞」、小分類見出しは「小分類名 件数 ＞」の並び。最内アイテムは「アイコン 項目名」のみで＞を付けない。
